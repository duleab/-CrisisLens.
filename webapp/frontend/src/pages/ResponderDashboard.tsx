...
import { getCrisisIcon, getSeverityStyle } from '../constants'
...
// Timeline icons now come from getCrisisIcon() in constants.ts
...
export default function ResponderDashboard({ events, stats, darkMode, region = 'sea' }: Props) {
              ...
              <div className="space-y-2">
                {topEvents.slice(0, 4).map(e => {
                  ...
                  <div key={e.id} className={`flex items-start gap-2 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    <span className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center" style={{ backgroundColor: sev.color }}>
                      {createElement(IconComp, { size: 12, color: 'white' })}
                    </span>
