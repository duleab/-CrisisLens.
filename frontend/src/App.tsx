...
export default function App() {
  ...
  const loadData = useCallback(async (isManual = false) => {
    try {
      setLoading(true)
      const [evData, statsData] = await Promise.all([fetchEvents(), fetchStats()])
  ...
  const renderDashboard = () => {
    switch (role) {
      case 'government':
        return <GovernmentDashboard events={events} stats={stats} darkMode={darkMode} />
      case 'responder':
        return <ResponderDashboard events={events} stats={stats} darkMode={darkMode} />
      case 'tourist':
        return <TouristDashboard events={events} stats={stats} darkMode={darkMode} />
      default:
        return <CitizenDashboard events={events} stats={stats} darkMode={darkMode} loading={loading} onIngest={handleIngest} />
    }
  }
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      {/* Sidebar */}
      <Sidebar role={role} darkMode={darkMode} events={events} />
